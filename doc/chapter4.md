...
_update方法的第一个参数是一个VNode对象，在内部会将该VNode对象与之前旧的VNode对象进行__patch__,用以达到针对最小差异修改进行渲染的目的。

patch方法是在Vue源码```src/platform/runtime/index.js```中赋为原型方法的，目的是获取不同的特有信息用以适配不同的平台。

```javascript
Vue.prototype.__patch__ = inBrowser ? patch : noop
```

至于```vm._render()```是在Vue源码```src/core/instance/render```中定义的，其作用是根据编译器编译好的render和staticRenerFns创建并返回一个VNode节点。下面看下源码。
```javascript
...
```
render和staticRenderFns是在后端渲染时传入的。

Vue源码```src/server/render.js```
```javascript
const normalizeRender = vm => {
  const { render, template } = vm.$options
  if (isUndef(render)) {
    if (template) {
      //获取render和staticRenderFns
      const renderFns = (
        compilationCache[template] ||
        (compilationCache[template] = compileToFunctions(template))
      )
      //render和staticRenderFns赋给vm.$options,用以传给vm._render()
...
patch将新旧虚拟DOM树进行比对，然后将根据两者的比较结果进行最小单位地修改视图，而不是将整个视图根据新的虚拟DOM树重绘。patch的核心在于diff算法，这套算法可以高效地比较virtual DOM的变更，从而得出变化以修改视图。
...
这两张图代表旧的虚拟DOM树与新虚拟DOM树进行patch的过程，因为他们只是在同层级的VNode之间进行比较差异（第二张图中相同颜色的方块代表互相进行比较的VNode节点），然后修改变化的视图，所以十分高效。
...
当两个VNode的tag、key、isComment都相同，并且同时定义或未定义data的时候，且如果标签为input则type必须相同(input的type可以为text，也可以为button,所以必须一致)。这时候这两个VNode则算sameVnode，可以直接进行patchVnode操作。

## patchVnode

patchVnode的关键是在新旧VNode节点均有子节点的情况下对子节点的遍历patch,至于其他的几种情况则可以不用太关注。
...
updateChildren方法不断往下层遍历并对同层新旧VNode节点patch,除了对真实DOM节点进行添加和删除操作之外，还要对真实DOM进行排序。
...
首先，在新老两个VNode节点的首尾两侧都有一个变量标记，在遍历过程中这几个变量都会向中间靠拢。当oldStartIdx > oldEndIdx或者newStartIdx > newEndIdx时结束循环。
...
如果以上情况均不符合，则通过createKeyToOldIdx会得到一个oldKeyToIdx，里面存放了一个key为旧的VNode，value为对应index序列的哈希表。从这个哈希表中可以找到是否有与newStartVnode一致key的旧的VNode节点，如果同时满足sameVnode，patchVnode的同时会将这个真实DOM（elmToMove）移动到oldStartVnode对应的真实DOM的前面。通俗的话将就是前面首尾匹配的四种情况下没有匹配成功的话，则拿newStartVnode节点去匹配剩下的中间顺序的节点，如果匹配上就patchVnode并移动真实DOM的位置。
...
由于Vue使用了虚拟DOM，所以虚拟DOM可以在任何支持JavaScript语言的平台上操作，例如说目前Vue支持的浏览器平台或是weex，它们在虚拟DOM的实现上是一致的。那么最后虚拟DOM如何映射到真实的DOM节点上呢？
...
同理，也会根据不同平台有自己不同的实现，我们这里以Web平台为例。Web平台的钩子函数见Vue源码```src/platform/runtime/modules```,里面有对attr、class、props、events、style以及transition（过渡状态）的DOM属性进行操作。
